...
#include "lcd.h"
#include "text.h"	
//Mini STM32开发板  RTC实时时钟 驱动代码		正点原子@ALIENTEK  2010/6/6
#define DateX 190		   //星期 显示X坐标
 #define HZSIZE	 	24
#define DATEX	 	10
#define DATEY	 	10 
...
//	BKP_WriteBackupRegister(BKP_DR1, 0X505a);	  //  yd 向指定的后备寄存器中写入用户程序数据
	if (BKP_ReadBackupRegister(BKP_DR1) != 0x5050)		//从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
		...
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set(2019,10,22,23,07,55);  //设置时间	
...
//初始化闹钟		  以1970年1月1日为基准  1970~2099年为合法年份
...
void firsttimedisplay(void) {   //第一次显示 时间  2019-11-1
	u8 t;
	  POINT_COLOR=BLUE;    //	Show_Str(30,230,200,24,"对应汉字为:",24,0);   //显示汉字的函数
		if(t!=calendar.sec)  { //---------------------在首行显示日期汉字 ------------------
				t=calendar.sec;
				...
				switch(calendar.week)
				{
					...
					case 1:
						...
						break;
					case 2:
						Show_Str(DateX,DATEY,72,24,"礼拜二",24,0);
						break;
					case 3:
						Show_Str(DateX,DATEY,72,24,"礼拜三",24,0);
						break;
					case 4:
						Show_Str(DateX,DATEY,72,24,"礼拜四",24,0);
						break;
					case 5:
